...
function StudyPlan(plan, id, type, totalCredits) {
  ...
  this.minCredits = type === "full-time" ? 60 : 20;
  ...
  if (plan) {
    plan.forEach((c) => {
      ...
      if (c.preparatory) this.required.push(c.preparatory);
    });
  }

  this.neededCredits = () => {
    return this.totalCredits - this.minCredits;
  };
  this.isInPlan = (code) => {
  ...
  this.isIncompatible = (code) => {
    return this.courses.filter(
      (c) => c.incompatible && c.incompatible.some((c) => c === code)
    );
  };
  this.limitationCourse = (course) => {
    const limitations = {
      required: !this.isDeletable(course.code)
        ? {
            courses: this.courses.filter((c) =>
              c.preparatory.some((ci) => ci === course.code)
            ),
          }
        : false,
      incompatible: this.incompatible.some((c) => c === course.code)
        ? {
            courses: this.courses.filter((c) =>
              c.incompatible.some((ci) => ci === course.code)
            ),
          }
        : false,
      full: course.isFull(),
